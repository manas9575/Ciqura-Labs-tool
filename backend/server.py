...
async def admin_create_user(req: AdminCreateUser, request: Request):
    ...
    # Including all required fields to satisfy MongoDB Schema Validation
    new_user = {
        "user_id": user_id,
        "display_id": display_id,
        "email": email,
        ...
        "role": req.role,
        "created_at": datetime.now(timezone.utc).isoformat(),
        # Fields required by your database schema:
        "picture": None,
        "phone": "",
        "address": "",
        "dob": "",
...
async def generate_receipt(payment_id: str, request: Request):
    ...
    settings = await db.institute_settings.find_one({}, {"_id": 0}) or {
